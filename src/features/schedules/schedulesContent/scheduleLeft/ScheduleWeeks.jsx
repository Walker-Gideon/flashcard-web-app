              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {sessionsByDay[day.date]?.map((session) => (
                    <div
                      key={session.id}
                      className={`flex items-center space-x-2 rounded-lg px-3 py-1 text-xs ${day.isToday ? `bg-slate-300 text-slate-800 dark:bg-slate-300/30 dark:text-slate-200` : `bg-slate-200 text-slate-700 dark:bg-slate-900/30 dark:text-slate-300`}`}
                    >
                      <div
                        className={`h-2 w-2 rounded-full ${day.isToday ? `bg-gradient-to-r from-slate-600 to-slate-700 dark:from-slate-200 dark:to-slate-300` : `bg-gradient-to-r from-slate-200 to-slate-300 dark:from-slate-600 dark:to-slate-700`}`}
                      ></div>

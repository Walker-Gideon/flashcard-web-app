...
export default function LargeHeader() {
  ...
  return (
    <header
      className={`transitioningColors flex flex-col gap-4 border-b border-stone-300 px-2 py-2 dark:border-slate-700`}
    >
      <div className="transitioningColors rounded-sm bg-slate-50 px-1 py-2 dark:bg-slate-700/50">
        <div className={`flex items-center justify-between`}>
          <div className="flex items-center gap-1 font-bold text-slate-950 uppercase">
            <div />
            <Logo logo={true} classname="z-50" />
            <span
              className={`medium:text-base transitioning transitioningColors z-30 text-sm font-bold dark:text-slate-50 ${resize ? "translate-x-0 delay-75" : "-translate-x-[150px]"} inline-block`}
            >
              walkwise
            </span>
          </div>
